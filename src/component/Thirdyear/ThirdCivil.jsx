...
export const ThirdCivil = () => {
  ...
  const fetchDataAsync2 = async () => {
    try {
      const response = await fetch(
        "http://127.0.0.1:8000/student_registration/student_list_new_third_civil"
      );
      if (!response.ok) {
  ...
  const handleAddFormSubmit = async (event) => {
    ...
    try {
      await fetch(
        "http://127.0.0.1:8000/student_registration/add_student_new_third_civil_admin",
        {
          method: "POST",
          body: formData,
        }
  ...
  const fetchDataAsync = async () => {
    try {
      const response = await fetch(
        "http://127.0.0.1:8000/student_registration/student_list_third_civil"
      );
      if (!response.ok) {
        throw new Error("Network response was not ok.");
      }
  ...
  const handleAddFormSubmit1 = async (event) => {
    ...
    try {
      await fetch(
        "http://127.0.0.1:8000/student_registration/add_student_old_third_civil_admin",
        {
          method: "POST",
          body: formData,
        }
  ...
  const handleDeleteClick = async (studentId) => {
    try {
      const response = await fetch(
        `http://127.0.0.1:8000/student_registration/delete_document_new_third_civil/${studentId}`,
        {
          method: "DELETE",
        }
      );
  ...
  const handleEditFormSubmit = async (event) => {
    ...
    try {
      ...
      const response = await fetch(
        `http://127.0.0.1:8000/student_registration/update_document_New_third_civil/${editContactId}`,
        {
          method: "PATCH",
  ...
  const handleEditFormSubmit1 = async (event) => {
    ...
    try {
      ...
      const response = await fetch(
        `http://127.0.0.1:8000/student_registration/update_document_old_third_civil/${editContactId1}`,
        {
          method: "PATCH",
          body: JSON.stringify(updateData),
        }
  ...
  const handleDeleteClick1 = async (studentId) => {
    try {
      const response = await fetch(
        `http://127.0.0.1:8000/student_registration/delete_document_old_third_civil/${studentId}`,
        {
          method: "DELETE",
        }
      );
  ...
  const handleViewClick = async (studentId) => {
    try {
      const response = await fetch(
        `http://127.0.0.1:8000/student_registration/view_third_civil_new/${studentId}`
      );
      if (!response.ok) {
        throw new Error("Network response was not ok.");
      }
                      ...
                      <td>
                        {editContactId === item._id ? (
                          ...
                          <>
                            ...
                            <button onClick={() => handlePaymentClick(item)}>
                              Payment
                            </button>
              ...
              ? filteredData.map((item, index) => (
                  <Fragment key={index}>
                    <tr>
                      ...
                      <td>
                        {editContactId1 === item._id ? (
                          <button onClick={handleEditFormSubmit1}>Save</button>
                        ) : (
                          <>
                            <button onClick={() => handleEditClick1(item)}>
                              Edit
                            </button>
                            <button
      ...
      {isRegisterFormVisible && (
        <div className="register-form-overlay">
          <div className="register-form-container">
            ...
            <ThirdView isVisible={true} data={viewedData} />
          </div>
        </div>
      )}
