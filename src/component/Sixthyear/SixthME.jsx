...
export const SixthME = () => {
  ...
  const fetchDataAsync2 = async () => {
    try {
      const response = await fetch(
        "http://127.0.0.1:8000/student_registration/student_list_new_sixth_me"
      );
      if (!response.ok) {
  ...
  const handleAddFormSubmit = async (event) => {
    ...
    try {
      await fetch(
        "http://127.0.0.1:8000/student_registration/add_student_new_sixth_me_admin",
        {
          method: "POST",
          body: formData,
        }
  ...
  const fetchDataAsync = async () => {
    try {
      const response = await fetch(
        "http://127.0.0.1:8000/student_registration/student_list_old_sixth_me"
      );
      if (!response.ok) {
        throw new Error("Network response was not ok.");
      }
  ...
  const handleAddFormSubmit1 = async (event) => {
    ...
    try {
      await fetch(
        "http://127.0.0.1:8000/student_registration/add_student_old_sixth_me_admin",
        {
          method: "POST",
          body: formData,
        }
  ...
  const handleDeleteClick = async (studentId) => {
    try {
      const response = await fetch(
        `http://127.0.0.1:8000/student_registration/delete_document_new_sixth_me/${studentId}`,
        {
          method: "DELETE",
        }
      );
  ...
  const handleEditFormSubmit = async (event) => {
    ...
    try {
      ...
      const response = await fetch(
        `http://127.0.0.1:8000/student_registration/update_document_new_sixth_me/${editContactId}`,
        {
          method: "PATCH",
  ...
  const handleEditFormSubmit1 = async (event) => {
    ...
    try {
      ...
      const response = await fetch(
        `http://127.0.0.1:8000/student_registration/update_document_old_sixth_me/${editContactId1}`,
        {
          method: "PATCH",
          body: JSON.stringify(updateData),
        }
  ...
  const handleDeleteClick1 = async (studentId) => {
    try {
      const response = await fetch(
        `http://127.0.0.1:8000/student_registration/delete_document_old_sixth_me/${studentId}`,
        {
          method: "DELETE",
        }
      );
  ...
  const handleViewClick = async (studentId) => {
    try {
      const response = await fetch(
        `http://127.0.0.1:8000/student_registration/view_sixth_me_new/${studentId}`
      );
      if (!response.ok) {
        throw new Error("Network response was not ok.");
      }
      ...
      {isRegisterFormVisible && (
        <div className="register-form-overlay">
          <div className="register-form-container">
            ...
            <SixthView isVisible={true} data={viewedData} />
          </div>
        </div>
      )}
