...
export const SecondEP = () => {
  ...
  const fetchDataAsync2 = async () => {
    try {
      const response = await fetch(
        "http://127.0.0.1:8000/student_registration/student_list_new_second_ep"
      );
      if (!response.ok) {
  ...
  const handleAddFormSubmit = async (event) => {
    ...
    try {
      await fetch(
        "http://127.0.0.1:8000/student_registration/add_student_new_second_ep_admin",
        {
          method: "POST",
          body: formData,
        }
  ...
  const fetchDataAsync = async () => {
    try {
      const response = await fetch(
        "http://127.0.0.1:8000/student_registration/student_list_old_second_ep"
      );
      if (!response.ok) {
        throw new Error("Network response was not ok.");
      }
  ...
  const handleAddFormSubmit1 = async (event) => {
    ...
    try {
      await fetch(
        "http://127.0.0.1:8000/student_registration/add_student_old_second_ep_admin",
        {
          method: "POST",
          body: formData,
        }
  ...
  const handleDeleteClick = async (studentId) => {
    try {
      const response = await fetch(
        `http://127.0.0.1:8000/student_registration/delete_document_new_second_ep/${studentId}`,
        {
          method: "DELETE",
        }
      );
  ...
  const handleEditFormSubmit = async (event) => {
    ...
    try {
      ...
      const response = await fetch(
        `http://127.0.0.1:8000/student_registration/update_document_new_second_ep/${editContactId}`,
        {
          method: "PATCH",
  ...
  const handleEditFormSubmit1 = async (event) => {
    ...
    try {
      ...
      const response = await fetch(
        `http://127.0.0.1:8000/student_registration/update_document_old_second_ep/${editContactId1}`,
        {
          method: "PATCH",
          body: JSON.stringify(updateData),
        }
  ...
  const handleDeleteClick1 = async (studentId) => {
    try {
      const response = await fetch(
        `http://127.0.0.1:8000/student_registration/delete_document_old_second_ep/${studentId}`,
        {
          method: "DELETE",
        }
      );
