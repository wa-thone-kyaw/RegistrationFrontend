...
export const Newstudent = () => {
  ...
  const fetchDataAsync2 = async () => {
    try {
      const response = await fetch(
        "http://127.0.0.1:8000/student_registration/student_list_new_first_civil"
      );
      if (!response.ok) {
  ...
  const handleAddFormSubmit = async (event) => {
    ...
    try {
      await fetch(
        "http://127.0.0.1:8000/student_registration/add_student_new_first_civil1_admin",
        {
          method: "POST",
          body: formData,
        }
  ...
  const fetchDataAsync = async () => {
    try {
      const response = await fetch(
        "http://127.0.0.1:8000/student_registration/exampassed_list"
      );
      if (!response.ok) {
        throw new Error("Network response was not ok.");
      }
  ...
  const handleAddFormSubmit1 = async (event) => {
    ...
    formData.append("nrc1", nrc1);
    formData.append("phone_no1", phone_no1);

    try {
      await fetch(
        "http://127.0.0.1:8000/student_registration/add_student_new_first_civil_admin",
        {
          method: "POST",
          body: formData,
        }
  ...
  const handleDeleteClick = async (studentId) => {
    try {
      const response = await fetch(
        `http://127.0.0.1:8000/student_registration/delete_document_new_first_civil/${studentId}`,
        {
          method: "DELETE",
        }
      );
  ...
  const handleEditFormSubmit = async (event) => {
    ...
    try {
      const updateData = {
        ...
        nrc: data2.find((item) => item._id === editContactId).nrc,
        phone_no: data2.find((item) => item._id === editContactId).phone_no,
      };

      const response = await fetch(
        `http://127.0.0.1:8000/student_registration/update_document_new_first_civil/${editContactId}`,
        {
          method: "PATCH",
  ...
  const handleEditFormSubmit1 = async (event) => {
    ...
    try {
      const updateData = {
        ...
        nrc1: data.find((item) => item._id === editContactId1).nrc1,
        phone_no1: data.find((item) => item._id === editContactId1).phone_no1,
      };

      const response = await fetch(
        `http://127.0.0.1:8000/student_registration/update_document_exam_result/${editContactId1}`,
        {
          method: "PATCH",
          body: JSON.stringify(updateData),
        }
  ...
  const handleDeleteClick1 = async (studentId) => {
    try {
      const response = await fetch(
        `http://127.0.0.1:8000/student_registration/delete_document_exam_result/${studentId}`,
        {
          method: "DELETE",
        }
      );
  ...
  const handlePaymentClick = (data2) => {
    ...
    setPaymentInfo({
      ...
      nrc: data2.nrc, // Assuming currentseat_no is the roll number
      email: data2.email,
      fee: data2.fee,
    });
  ...
  const handleViewClick = async (studentId) => {
    try {
      const response = await fetch(
        `http://127.0.0.1:8000/student_registration/view_second_civil_new/${studentId}`
      );
      if (!response.ok) {
        throw new Error("Network response was not ok.");
      }
                      ...
                      <td>
                        {editContactId === item._id ? (
                          <input
                            ...
                            value={item.nrc}
                            onChange={(e) =>
                              handleEditFormChange(e, "nrc", item._id)
                            }
                          />
                        ) : (
                          item.nrc
                      ...
                      <td>
                        {editContactId1 === item._id ? (
                          <input
                            ...
                            value={item.nrc1}
                            onChange={(e) =>
                              handleEditFormChange1(e, "nrc1", item._id)
                            }
                          />
                        ) : (
                          item.nrc1
      ...
      {isRegisterFormVisible && (
        <div className="register-form-overlay">
          <div className="register-form-container">
            ...
            <FirstView isVisible={true} data={viewedData} />
          </div>
        </div>
      )}
      {isPaymentPopupVisible && (
        <div className="payment-popup">
          <div className="payment-popup-content">
            ...
            <p>Roll Number: {paymentInfo.nrc}</p>
