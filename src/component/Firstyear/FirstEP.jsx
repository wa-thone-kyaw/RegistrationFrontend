...
export const FirstEP = () => {
  ...
  const fetchDataAsync = async () => {
    try {
      const response = await fetch(
        "http://127.0.0.1:8000/student_registration/first_ep_list"
      );
      if (!response.ok) {
  ...
  const handleAddFormSubmit1 = async (event) => {
    ...
    try {
      await fetch(
        "http://127.0.0.1:8000/student_registration/add_student_old_first_ep",
        {
          method: "POST",
          body: formData,
        }
  ...
  const handleEditFormSubmit1 = async (event) => {
    ...
    try {
      ...
      const response = await fetch(
        `http://127.0.0.1:8000/student_registration/update_document_old_first_ep/${editContactId1}`,
        {
          method: "PATCH",
  ...
  const handleDeleteClick1 = async (studentId) => {
    try {
      const response = await fetch(
        `http://127.0.0.1:8000/student_registration/delete_document_old_first_ep/${studentId}`,
        {
          method: "DELETE",
        }
      );
